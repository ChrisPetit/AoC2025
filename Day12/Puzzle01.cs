                ...
                public static class Puzzle01
                {
                    ...
                    private static string[] NormalizeLines(string[]? lines)
                    {
                        ...
                        for (var i = 0; i < lines.Length; i++)
                            result[i] = (lines[i] ?? string.Empty).TrimEnd('\r');
                        return result;
                    }
                    ...
                    private static List<RegionRequest> ParseRegions(string[] lines, int startIndex, int shapeCount)
                    {
                        ...
                        for (var i = startIndex; i < lines.Length; i++)
                        {
                            ...
                            var dimensionTokens = sizePart.Split('x', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                    ...
                    private static bool CanFitAll(RegionRequest region, IReadOnlyList<ShapeDefinition> shapes)
                    {
                        ...
                        for (var i = 0; i < counts.Length; i++)
                            remainingArea += counts[i] * shapeAreas[i];
                            ...
                            foreach (var orientation in shape.Orientations)
                            {
                                ...
                                for (var offsetY = 0; offsetY <= region.Height - orientation.Height; offsetY++)
                                {
                                    for (var offsetX = 0; offsetX <= region.Width - orientation.Width; offsetX++)
                                    {
                                        ...
                                        Array.Sort(cells);
                                        perShape[shapeIndex].Add(new Placement(shapeIndex, cells));
                                    }
                                }
                            }
                        }
                    ...
                    private static bool IsShapeHeader(string line)
                    {
                        ...
                        if (colon < 0)
                            return false;
                        return int.TryParse(line[..colon].Trim(), out _);
                    }
                    ...
                    private static bool IsRegionHeader(string line)
                    {
                        ...
                        if (colon < 0)
                            return false;
                        return line[..colon].IndexOf('x', StringComparison.OrdinalIgnoreCase) >= 0;
                    }
                
                    private sealed class ShapeDefinition
                    {
                        public ShapeDefinition(int index, ShapeOrientation[] orientations)
                        {
                            Index = index;
                            Orientations = orientations;
                            Area = orientations[0].Cells.Length;
                        }
                
                        public int Index { get; }
                        public ShapeOrientation[] Orientations { get; }
                        public int Area { get; }
                    }
                    ...
                    {
                        public Placement(int shapeIndex, int[] cells)
                        {
